...
- **Validator suggestions** — where a named tool produces the evidence for a criterion, it is listed on the card with a link and how it is run (in the app, locally, or as a hosted service), resolved from `validators.json` through `lib/validators.js`. The registry ships 14 validators and three community registries (FAIRsharing, DCC, RDA MSC) for the long tail
